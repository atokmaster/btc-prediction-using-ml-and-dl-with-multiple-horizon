# === Train 1D Horizon from 4H and 1D datasets (ML + DL) ===
...
batch_size = 32
units_list = [16, 32, 64]
base_dir = "G:/BTC_AI_SIgnal/loop_results/Horizon_1D"
...
def train_ml_model(data_path, source_tag):
    ...
    for name, model in models.items():
        ...
        model_path = os.path.join(model_dir, f"{name}_model_1d_from_{source_tag}.pkl")
        joblib.dump(model, model_path)
        save_features_json(model_dir, feature_cols, len(feature_cols))
        result.append({
            "Model_Name": f"{name.upper()}_1D",
            "Model_Type": "ML",
            "Accuracy": round(acc, 4),
            "F1_Macro": round(f1, 4),
            "Source": source_tag.upper(),
            "Architecture": name.upper(),
            "Model_Path": f"Horizon_1D/from_{source_tag}/ML/{name}/{name}_model_1d_from_{source_tag}.pkl",
            "Feature_Len": len(feature_cols),
            "Epoch_Used": "-", "Sequence_Length": "-", "Dropout": "-"
        })
    return result

# === DL CNN-1D for 4H only ===
def train_cnn1d_from_4h(data_path):
    ...
    for units in [32, 64]:
        ...
        model_name = f"cnn1d{units}_model_ep{used_epoch}_1d_from_4h.keras"
        model_dir = os.path.join(base_dir, "from_4h/DL/cnn1d")
        os.makedirs(model_dir, exist_ok=True)
        model.save(os.path.join(model_dir, model_name))
        save_features_json(model_dir, feature_cols, len(feature_cols))
        result.append({
            "Model_Name": f"CNN1D{units}_1D",
            "Model_Type": "DL",
            "Accuracy": val_acc,
            "F1_Macro": f1_macro,
            "Source": "4H",
            "Architecture": f"CNN1D_{units}",
            "Model_Path": f"Horizon_1D/from_4h/DL/cnn1d/{model_name}",
            "Feature_Len": len(feature_cols),
...
# === DL from 1D (LSTM, GRU, CNN-LSTM, Transformer) ===
def train_deep_models_from_1d(data_path):
    df = pd.read_excel(data_path).dropna(subset=[label_col])
    feature_cols = [col for col in df.columns if col not in ['datetime'] and not col.startswith('return_') and not col.startswith('label_')]
    X = df[feature_cols].values
    y = df[label_col].values.astype(int)
    ...
    for name, build_fn in builders.items():
        model_dir = os.path.join(base_dir, f"from_1d/DL/{name}")
        os.makedirs(model_dir, exist_ok=True)
        save_features_json(model_dir, feature_cols, len(feature_cols))
        for units in units_list:
            ...
            y_pred = model.predict(X_test, verbose=0)
            f1_macro = round(f1_score(np.argmax(y_test_cat, axis=1), np.argmax(y_pred, axis=1), average='macro'), 4)
            fname = f"{name}{units}_model_ep{used_epoch}_1d_from_1d.keras"
            model.save(os.path.join(model_dir, fname))
            result.append({
                "Model_Name": f"{name.upper()}{units}_1D",
                "Model_Type": "DL",
                "Accuracy": val_acc,
                "F1_Macro": f1_macro,
                "Source": "1D",
                "Architecture": f"{name.upper()}_{units}",
                "Model_Path": f"Horizon_1D/from_1d/DL/{name}/{fname}",
                "Feature_Len": len(feature_cols),
                "Epoch_Used": used_epoch,
                "Sequence_Length": seq_len,
                "Dropout": dropout
...
results += train_cnn1d_from_4h("G:/BTC_AI_SIgnal/data/btc_4h_final.xlsx")
results += train_ml_model("G:/BTC_AI_SIgnal/data/btc_1d_final.xlsx", "1d")
results += train_deep_models_from_1d("G:/BTC_AI_SIgnal/data/btc_1d_final.xlsx")

# === Save final_comparison_1D.csv ===
df_result = pd.DataFrame(results)
csv_path = os.path.join(base_dir, "final_comparison_1D.csv")
